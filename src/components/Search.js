...
class Search extends React.Component {
  ...
  // curl -u crio.do:learnbydoing http://65.2.46.17:8082/api/v1/products;
   ...
   */
  getProducts = async () => {
    let perform_APICall = await this.performAPICall();
    //let clone = perform_APICall;
    if (perform_APICall) {
  ...
  search = (text) => {
    ...
    if(text.length===0){
      ...
      result = this.products.filter(function(item){
        ...
        return (nm.includes(text) || cat.includes(text));
      });
    } 
  ...
  debounceSearch = (event) => {
    ...
    if(this.debounceTimeout)
    {
      ...
      //this.func(...this.search(searchText));
    }
        ...
        <Product
          ...
          addToCart={() => {
            if (this.state.loggedIn) {
             ...
             this.refs.cartRef.pushToCart(product._id,1);
            }
            else{
    ...
    return (
      <>
        ...
        <Row>
          {/* Display products */}
          {/* {this.state.loggedIn ? */}
          <Col xs={{ span : 24 }} 		md={{ span: this.state.loggedIn && this.products.length ? 18 : 24 }}
            >
          ...
          {this.state.loggedIn && this.products.length && (
            ...
            >
              ...
              <Cart history = {this.props.history} ref={"cartRef"} products = {this.products}/>
              </div>
            </Col>
          )}
        </Row>
